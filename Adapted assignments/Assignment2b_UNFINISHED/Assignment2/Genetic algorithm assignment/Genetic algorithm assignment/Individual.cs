...
namespace Genetic_algorithm_assignment
{
    class Individual: IComparable<Individual>
    {
        ...
        public float Fitness
        {
            get
            {
                var decodedValue = this.DecodeGenes();
                var resultOp = (decodedValue["b"] / (decodedValue["a"] + decodedValue["b"] + decodedValue["c"])) - decodedValue["e"] - (decodedValue["f"] / decodedValue["d"]);
                return 1f / (1f + Math.Abs(resultOp - 13f));
            }
        }
        ...
        {
            if (genes.Length != 15)
                throw new Exception("An individual should always have 13 genes!");
            this.genes = genes;
        }

        public static Individual CreateRandomIndividual()
        {
            byte[] genes = new byte[15];
        ...
        public Dictionary<string, int> DecodeGenes()
        {
            string[] valuesKeys = new string[] { "a", "b", "c", "d", "e" };
            var decodedValues = new Dictionary<string, int>();
            for (int i = 0; i < valuesKeys.Length; i++)
            {
                ...
                {
                    if (genes[j] == 1)
                        decodedValue += (int)Math.Pow(2, j - (i * 3));
                }
                decodedValues.Add(valuesKeys[i], decodedValue);
            }
